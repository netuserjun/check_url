...
def virustotal_h(hash_value):
        ...
        if result.get('response_code') == 0:
                ...
                print(str(scans_items))
                if 'True' not in str(scans_items):
...
def virustotal_f(fn):
        ...
        scans_items = report.get('scans').items()
        #scans_detected_value = list(scans_items)[0][1]['detected']      # 바이러스토탈에서 매칭되는 값 있음 -> 디텍트 값 True / False
        #print('scans_detected_value : '+ str(scans_detected_value))

        if 'True' not in str(scans_items):
        #if str(scans_detected_value) == 'False':
                string1 = "666666\n"  #바이러스토탈에서 파일 조회 -> 매칭되는 값 없음
                print("==========! No Virus in File-Data !==========")
                return string1,0
        else:
                #바이러스토탈에서 파일 조회 -> 매칭되는 값 있음
                print('==========! Virus found in File-Data !==========')
            ...
            #	download(url_decode,"tmp.zip")
                        ...
                        if result0 == '444444\n' or result0 == '999999\n':
                                ...
                                if result2 == '666666\n':
                                        ...
                                        string10 = '해당 URL은 클릭 시 파일이 다운로드됩니다.\n'
                                        string11 = '----- 세부 사항 -----\n'
                                        string12 = '해시값 /:/ 알려진 바이러스 해시값과 일치하는 항목 없음\n'
                                        string13 = '파일분석 /:/ 68개의 백신이 분석한 결과, 알려진 바이러스 검출되지 않음\n'

                                        client_socket.sendall(string6.encode())
                                        time.sleep(1)
                                        client_socket.sendall(string10.encode())
                                        client_socket.sendall(string11.encode())
                                        client_socket.sendall(string12.encode())
                                        client_socket.sendall(string13.encode()) 
                                        #string13이 들어가면 [분석 결과 보기]btn 이미지 띄우기
                                else:
...
# print(r[url_name]['threats'])
                if r[url_name]['malicious'] == False :
                        ...
                        try:
                                ...
                                # client_socket.sendall(string.encode())
                                time.sleep(0.5)
                                # hi=countryCode.encode()
                                string_IP = "IP : " + ip + "\n"
                                string100 = "Country_Code : " + countryCode
                                client_socket.sendall(string_IP.encode())
                                ...
                                time.sleep(0.5)
                                string_IP = "IP : " + ip + "\n"
                                string101 = "Country_Code : " + string2
                                client_socket.sendall(string_IP.encode())
                                client_socket.sendall(string101.encode())
                                client_socket.sendall(string.encode())
                                time.sleep(0.5)
                                goto .end
                else :

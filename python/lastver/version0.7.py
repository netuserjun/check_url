...
def virustotal_h(hash_value, fn):
        ...
        print('Response_json : '+ str(result))
        ...
        if result.get('response_code') == 0:
                ...
                for x in result['scans']:
                        #if result['scans'][x]['detected'] == 'Success':
...
def handle_client(client_socket, addr):
        ...
        if '</body>' not in str(check_body):
			...
			#make hash_value
                        ...
                        """
                        ha.search_hash('1d389849db67d50f48d30670e1e8437a97a33fdba75a31e75c06176b1cfb4a211')
                        ha.search_hashes(['1d389849db67d50f48d30670e1e8437a97a33fdba75a31e75c06176b1cfb44a21','bd3a86453614b1c1f72685d9393d6cbe63e1097267d12b19340c62870ce533e50'])
                        ha.search_terms({'filename': 'invoice.exe', 'tag': 'ransomware'})

                        ha.quick_scan_state()
                        ha.quick_scan_file('lookup_ha', 'xtgxutpf.js', '../xtgxutpf.js', options={'commeent': 'Bondat malware'})
                        ha.quick_scan_url_to_file('lookup_ha', 'url')
                        ha.quick_scan_url_for_analysis('all', 'https://www.google.com')
                        ha.quick_scan_id('scan_id')
                        """
                        ha.overview_sha256('sha256_data')
                        ha.overview_sha256_refresh('sha256_data')
                        ha.overview_sha256_summary('sha256_data')
                        ha.overview_sha256_sample('sha256_data')
                        
                        ha.submit_file(300, 'file_name', '/home/checkurl/python/file_name', options={'comment': 'Bondat malware'})
                        print(ha.submit_file)
                        ha.submit_url_to_file(300, 'url')
                        ha.submit_url_for_analysis(300, 'https://www.google.com')

...
def virustotal_f(fn):
        ...
        print('Response_Code : '+ str(report.get('response_code')))
        #print('Scans : '+ str(report['scans']))

        scans_items = report.get('scans').items()
        #scans_detected_value = list(scans_items)[0][1]['detected']      # 바이러스토탈에서 매칭되는 값 있음 -> 디텍트 값 True / False
        #print('scans_detected_value : '+ str(scans_detected_value))

        print('Scans_items : '+ str(scans_items))

        if 'True' not in str(scans_items):
        #if str(scans_detected_value) == 'False':
                string1 = "666666\n"  #바이러스토탈에서 파일 조회 -> 바이러스 미검출
                ...
                for x in report['scans']:
                        #print('True/Flase : '+ str(report['scans'][x]['detected']))
                        if str(report['scans'][x]['detected']) == 'True':
                                ...
                                #print('X : '+ x)
                                list_x.append(x)
                                #print('Result : '+ report['scans'][x]['result'])

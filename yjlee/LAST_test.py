...
# Virustotal에 파일에 바이러스가 검출된 경우, 디텍트 엔진 출력
...
def handle_client(client_socket, addr):
        ...
        if '</body>' not in str(check_body) or 'setRequestHeader' in str(check_body):
        ...
        #     #파일 다운로드하기
            ...
            #make hash_value
                        ...
                        dir, file_test = os.path.split('/home/checkurl/yjlee/bad.zip') 
                        file_name = file_test

                        result0,result1 = virustotal_h(sha256_data,file_name)
                        ...
                        if result0 == '444444\n' or result0 == '999999\n':
                                ...
                                if result2 == '666666\n':
                                        string6 = '바이러스토탈_파일_바이러스 미검출~\n'
                                        client_socket.sendall(string6.encode())
                                        time.sleep(2)
                                else:
                                        string7 = '바이러스토탈_파일_바이러스 검출!\n'
                                        ...
                                        client_socket.sendall(string_x.encode())
                                        time.sleep(2)
                                        client_socket.sendall(string_n.encode())
                                        time.sleep(7)
                                        client_socket.sendall(string_result.encode())
                                        time.sleep(2)
                        else:   
                                string = "바이러스토탈_엔진_Detected\n"
                                string_x = result0
                                ...
                                #type(result0) == "<class 'list'>" or type(result1) == "<class 'list'>":
